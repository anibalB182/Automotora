...
def listado(request):
    #consultaremso a la baxe de datos par decirle que me entregue
    #todos los automoviles
    
    autos = Automovil.objects.all()
    #llamamos al template y junto con ello le pasamos datos, en este caso
    #el listado de autos que exist en la bbdd
...
def formulario(request):
    ...
    #preguntamos si la peticion es POST
    if request.POST:
        ...
        #dejamos el objeto marca dentro de auto
        auto.marca = marca
        #teniendo todos los datos capturados desde el template,
        #guardamos el automovil en la bddd
        try:
            auto.save()
